...
def adapt_to_screen():
    if platform == 'android':
        ...
        width, height, density = get_screen_info()
        # Update dp scale based on device density
...
screen_width, screen_height = adapt_to_screen()
...
def get_download_path():
    ...
    if platform == 'android':
        # Use Android's public Download directory
        from android.storage import primary_external_storage_path
        return os.path.join(primary_external_storage_path(), 'Download', 'YTDownloader')
    else:
        # Use current directory for desktop
...
def get_ffmpeg_path():
    ...
    if platform == 'android':
        # Use the bundled ffmpeg in the app's private storage
...
class YouTubeDownloaderApp(App):
    def __init__(self, **kwargs):
        ...
        self.has_storage_permission = False
    ...
    def build(self):
        ...
        # Adjust UI scaling based on screen size
        width, height = screen_width, screen_height
        base_width = 480  # reference width
        self.scale_factor = width / base_width
        
        # Enable multitouch
        Window.softinput_mode = 'below_target'  # Prevent keyboard from covering input
    ...
    def request_android_permissions(self):
        ...
        if platform == 'android':
            ...
            def callback(permissions, grant_results):
                if all(grant_results):
                    self.has_storage_permission = True
                    # Create download directory if it doesn't exist
                    download_dir = os.path.join(primary_external_storage_path(), 'Download', 'YTDownloader')
        ...
        # Adjust dp function for consistent sizing
        from functools import partial
        self.sdp = partial(dp, scale=self.scale_factor)
